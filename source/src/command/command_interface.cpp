...
CommandInterface::CommandInterface(const std::string& config_path, std::shared_ptr<HardwareInterface> hw_interface_ptr,
...
void CommandInterface::filter_command() {
    ...
    for (size_t i = 0; i < 3; i++) {
        fsm_data_ptr_->desired_command_ptr->base_pos[i] = first_order_filter(
            fsm_data_ptr_->desired_command_ptr->base_pos[i], des_raw_command_.base_pos[i], loco_pos_filter_para_[i]);
        fsm_data_ptr_->desired_command_ptr->base_lin_vel[i] =
            first_order_filter(fsm_data_ptr_->desired_command_ptr->base_lin_vel[i], des_raw_command_.base_lin_vel[i],
                               loco_vel_filter_para_[i]);
        fsm_data_ptr_->desired_command_ptr->base_rpy[i] = first_order_filter(
            fsm_data_ptr_->desired_command_ptr->base_rpy[i], des_raw_command_.base_rpy[i], loco_ori_filter_para_[i]);
        fsm_data_ptr_->desired_command_ptr->base_ang_vel[i] =
            first_order_filter(fsm_data_ptr_->desired_command_ptr->base_ang_vel[i], des_raw_command_.base_ang_vel[i],
...
void CommandInterface::clamp_command() {
    for (size_t i = 0; i < 3; i++) {
        des_raw_command_.base_pos[i] = clamp(des_raw_command_.base_pos[i], pos_des_min_[i], pos_des_max_[i]);
        des_raw_command_.base_lin_vel[i] = clamp(des_raw_command_.base_lin_vel[i], vel_des_min_[i], vel_des_max_[i]);
        des_raw_command_.base_rpy[i] = clamp(des_raw_command_.base_rpy[i], ori_des_min_[i], ori_des_max_[i]);
        des_raw_command_.base_ang_vel[i] = clamp(des_raw_command_.base_ang_vel[i], ome_des_min_[i], ome_des_max_[i]);
    }
}
