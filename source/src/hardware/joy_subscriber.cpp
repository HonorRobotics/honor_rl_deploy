...
JoySubscriber::JoySubscriber(rclcpp::Node::SharedPtr node, const std::string& topic_name)
: node_(node)
{
...
bool JoySubscriber::warp_robot_remote_control_to_data(const sensor_msgs::msg::Joy::SharedPtr msg)
{
  ...
  if (msg->axes.size() < 4) {
    LOG(ERROR) << "**[Warning] axes dimension is insufficient, current size = "
               << msg->axes.size() << ", expected at least 4!**";
    return false;
  }
  if (msg->buttons.empty()) {
    LOG(ERROR) << "**[Warning] buttons dimension is insufficient, current size = "
               << msg->buttons.size() << ", expected at least 1!**";
    return false;
  }
...
bool JoySubscriber::warp_game_controller_to_data(const sensor_msgs::msg::Joy::SharedPtr msg)
{
  if (msg->axes.size() < 6) {
    LOG(ERROR) << "**[Warning] axes dimension is insufficient, current size = "
               << msg->axes.size() << ", expected at least 6!**";
    return false;
  }
  if (msg->buttons.size() < 21) {
    LOG(ERROR) << "**[Warning] buttons dimension is insufficient, current size = "
               << msg->buttons.size() << ", expected at least 21!**";
    return false;
  }
...
{
  const auto state_name = [](ButtonState state) {
    return state == ButtonState::PRESS ? "PRESS" : "RELEASE";
  };
