  ...
  const std::string& imu_topic_name)
{
  low_state_subscription_ = node->create_subscription<interaction_msgs::msg::LowState>(
    low_state_topic_name,
    10,
    std::bind(&LowStateSubscriber::low_state_callback, this, std::placeholders::_1));

  imu_subscription_ = node->create_subscription<sensor_msgs::msg::Imu>(
    imu_topic_name,
    10,
    std::bind(&LowStateSubscriber::imu_callback, this, std::placeholders::_1));
}
...
void LowStateSubscriber::low_state_callback(
  const interaction_msgs::msg::LowState::SharedPtr msg)
{
...
void LowStateSubscriber::print_low_state_msg(
  const interaction_msgs::msg::LowState& msg)
{
  LOG(INFO) << "┌──────────────────────────────";
