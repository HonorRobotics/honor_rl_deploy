  ...
  switch (key) {
    ...
    default:
      if (std::isprint(static_cast<unsigned char>(key))) {
        ...
        LOG(INFO) << "[Keyboard] ignored unrecognized key: 0x"
                   << std::hex << static_cast<int>(static_cast<unsigned char>(key));
      }
      break;
  }
}
...
std::shared_ptr<JoystickData> KeyboardInterface::get_latest_data()
{
  ...
  if (now - ts_lx_ < MOVE_HOLD) { data->lx = target_lx_; }
  if (now - ts_ly_ < MOVE_HOLD) { data->ly = target_ly_; }
  if (now - ts_rx_ < MOVE_HOLD) { data->rx = target_rx_; }
