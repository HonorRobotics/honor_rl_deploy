    ...
    rl_model_inference_thread_ = std::thread([&]() {
        ...
        while (controller_running_) {
            try {
                ...
                LOG(ERROR)
                    << "[FSMStateRLLocomotion::init_rl_model_inference_thread]: RL model inference thread error : "
                    << e.what();
            }
        }
    });
...
void FSMRLBase::head_control() {
    ...
    for (size_t i = 0; i < 3; ++i) {
        if (head_q_target[i] != last_head_q_cmd_[i]) {
            ...
            LOG(ERROR) << "Head desired q angle changed: the " << i << " joint new command is " << head_q_target[i]
                       << " rad, old command is " << last_head_q_cmd_[i] << " rad, the duration is " << head_q_duration[i] << " s";
        }
    ...
    for (size_t i = 0; i < fsm_data_ptr_->head_num_dofs; ++i) {
        head_q_des[i] = 0.5 * (head_q_init_[i] - head_q_target[i]) * std::cos(M_PI * phase[i]) +
                        0.5 * (head_q_init_[i] + head_q_target[i]);
        head_dq_des[i] =
            0.5 * (head_q_init_[i] - head_q_target[i]) * (-M_PI * std::sin(M_PI * phase[i]) / head_q_duration[i]);
    }
