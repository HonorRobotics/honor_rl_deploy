...
FSMStateRecoveryStand::FSMStateRecoveryStand(std::shared_ptr<FSMData> fsm_data_ptr)
    ...
    std::string config_path = "../config/" + fsm_data_ptr_->robot_name + "/" + fsm_data_ptr_->robot_version +
                              "/recovery_stand/recovery_stand.yaml";
...
void FSMStateRecoveryStand::run() {
    ...
    for (size_t i = 0; i < fsm_data_ptr_->num_dofs; ++i) {
        q_des[i] = 0.5 * (q_init_[i] - default_dof_pos_[i]) * std::cos(M_PI * phase) +
                   0.5 * (q_init_[i] + default_dof_pos_[i]);
...
void FSMStateRecoveryStand::read_yaml(const std::string& config_path) {
    try {
        ...
        throw std::runtime_error("Failed to load recovery stand config '" + config_path + "': " + e.what());
    }

    if (!std::isfinite(transition_time_) || transition_time_ <= 0.0) {
        throw std::runtime_error("Invalid transition_time in recovery stand config '" + config_path +
                                 "': expected a finite value greater than zero");
    }

    const Eigen::Index expected_size = fsm_data_ptr_->num_dofs;
    if (default_dof_pos_.size() != expected_size || kp_.size() != expected_size || kd_.size() != expected_size) {
        throw std::runtime_error("Invalid vector size in recovery stand config '" + config_path +
                                 "': default_dof_pos, kp and kd must each contain " + std::to_string(expected_size) +
                                 " values");
    }
}
