...
FSMStateDamper::FSMStateDamper(std::shared_ptr<FSMData> fsm_data_ptr)
  ...
  std::string config_path = "../config/" + fsm_data_ptr_->robot_name + "/" + fsm_data_ptr_->robot_version +"/damper/damper.yaml";
...
StateID FSMStateDamper::check_transition() {
  ...
  switch (state_id) {
    ...
    default:
      LOG(INFO) << "[CONTROL FSM] Bad Request: Cannot transition from " << StateID::DAMPER << " to "
                << state_id;
  }
