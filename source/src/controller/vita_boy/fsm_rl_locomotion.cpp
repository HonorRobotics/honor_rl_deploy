...
FSMStateRLLocomotion::FSMStateRLLocomotion(std::shared_ptr<FSMData> fsm_data_ptr)
  ...
  std::string config_path = "../config/" + fsm_data_ptr_->robot_name + "/" + fsm_data_ptr_->robot_version + "/rl_locomotion/rl_locomotion.yaml";
  init_rl_base(config_path);

  std::string policy_path = "../config/" + fsm_data_ptr_->robot_name + "/" + fsm_data_ptr_->robot_version + "/rl_locomotion/" + rl_params_.policy_name;
...
void FSMStateRLLocomotion::update_observation() {
  ...
  if (obs_ind != rl_params_.num_observations) {
    LOG(ERROR) << "[FSMStateRLLocomotion::update_observation]: ERROR! observation dimension not match, required " << rl_params_.num_observations << " but given "
              << obs_ind;
  }
  ...
  for (size_t term = 0; term < term_sizes.size(); ++term) {
    ...
    if (!history_initialized_) {
      for (int frame = 0; frame < num_hist; ++frame){
        std::copy(observation_begin, observation_begin + term_size,
                  history_begin + frame * term_size);
      }
    }
    else {
      if (num_hist > 1) {
        std::memmove(history_begin, history_begin + term_size,
                     (num_hist - 1) * term_size * sizeof(double));
      }
      std::copy(observation_begin, observation_begin + term_size,
                history_begin + (num_hist - 1) * term_size);
    }
