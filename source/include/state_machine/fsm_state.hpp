...
class FSMState {
   public:
    FSMState(std::shared_ptr<FSMData> fsm_data_ptr, StateID id, std::string name)
        : fsm_data_ptr_(fsm_data_ptr), state_id_(id), state_name_(std::move(name)) {}
    virtual ~FSMState() = default;
    virtual void onEnter() = 0;
    virtual void run() = 0;
    virtual void onExit() = 0;
    virtual StateID check_transition() {
        return state_id_;
    }
    const std::string& get_state_name() const {
        return state_name_;
    }
    const StateID& get_state_id() const {
        return state_id_;
    }
