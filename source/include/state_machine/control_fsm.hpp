    ...
    void print_info(int opt) {
        switch (opt) {
            ...
            if (print_iter_ == print_num_) {
                ...
                LOG(INFO) << "[FSM] Mode: TRANSITIONING from " << current_state_->get_state_name() << " to " << next_state_->get_state_name()
                            << "\n";
                }
            ...
            LOG(INFO) << "[FSM] Transition initialized from " << current_state_->get_state_name() << " to " << next_state_->get_state_name()
                        << "\n";
            break;

            case 2:  // Finalizing FSM State transition
            LOG(INFO) << "[FSM] Transition finalizing from " << current_state_->get_state_name() << " to " << next_state_->get_state_name()
                        << "\n";
            break;
        }
    }
...
public:
    ...
    virtual std::shared_ptr<FSMState> create_state(void *context, std::shared_ptr<FSMData> fsm_data_ptr, const std::string &state_name) = 0;
...
public:
    ...
    std::shared_ptr<FSM> create_FSM(const std::string &type, const std::string &version, std::shared_ptr<FSMData> fsm_data_ptr, void *context)
    {
