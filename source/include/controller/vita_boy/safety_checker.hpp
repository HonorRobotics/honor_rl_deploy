...
public:
    ...
    bool check_imu_quaternion_norm() {
      ...
      if (norm > 1.2 || norm < 0.8) {
        ...
        LOG(ERROR) << "[SAFETY CHECKER] failed! imu quaternion norm is larger or smaller than 1.0, the current norm is " << norm;
      ...
      if (euler_zyx(1) > 80.0 / 180.0 * M_PI || euler_zyx(1) < -80.0 / 180.0 * M_PI) {
        if (print_check_info_) {
          ...
          LOG(ERROR) << "[SAFETY CHECKER] Current quaternion wxyz is: "
                      ...
                      << coeffs[1] << ", "  // y
                      << coeffs[2];         // z
          LOG(ERROR) << "[SAFETY CHECKER] Pitch limit (-PI*80/180, PI*80/180), actual orientation Pitch angle: " << euler_zyx(1);
        }
        safety = false;
          ...
          if (print_check_info_) {
            ...
            LOG(ERROR) << "[SAFETY CHECKER] Current quaternion wxyz is: "
                        ...
                        << coeffs[1] << ", "  // y
                        << coeffs[2];         // z
            LOG(ERROR) << "[SAFETY CHECKER] Roll limit (-PI*50/180, PI*50/180), actual orientation Roll angle: " << euler_zyx(2);
          }
          safety = false;
        }
      return safety;
