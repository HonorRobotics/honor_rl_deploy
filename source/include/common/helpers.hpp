...
inline Eigen::Quaterniond add_roll_pitch_bias_to_quaternion(
...
shortest_angular_distance(T from, T to) {
    static_assert(std::is_floating_point<T>::value,
                 "shortest_angular_distance requires floating-point types");
...
template<typename T>
Eigen::Quaternion<T> quaternion_multiplication_optimized(const Eigen::Quaternion<T>& q1, const Eigen::Quaternion<T>& q2) {
    T w1 = q1.w();
    T x1 = q1.x();
    T y1 = q1.y();
    T z1 = q1.z();
...
inline Eigen::VectorXd forder_cos_smooth(const Eigen::VectorXd des_pos, const Eigen::VectorXd pos_init, double dt, int counter, double transition_time)
{
