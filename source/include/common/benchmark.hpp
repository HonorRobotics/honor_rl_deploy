...
class RepeatedTimer {
 public:
   ...
   */
  int get_num_timed_intervals() const { return num_timed_intervals_; }

  /**
   * @return Total cumulative time of timed intervals
   */
  double get_total_in_milliseconds() const { return std::chrono::duration<double, std::milli>(total_time_).count(); }

  /**
   * @return Maximum duration of a single interval
   */
  double get_max_interval_in_milliseconds() const { return std::chrono::duration<double, std::milli>(max_interval_time_).count(); }

  /**
   * @return Duration of the last timed interval
   */
  double get_last_interval_in_milliseconds() const { return std::chrono::duration<double, std::milli>(last_interval_time_).count(); }

  /**
   * @return Average duration of all timed intervals
   */
  double get_average_in_milliseconds() const { return get_total_in_milliseconds() / num_timed_intervals_; }
